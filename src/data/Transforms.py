...
def get_transforms(train: bool = True, gamma: float = 1.0) -> A.Compose:
    ...
    if train:
        transforms_list.extend([
            ...
            A.CoarseDropout(max_holes=40, max_height=15, max_width=15, p=0.8),
            A.RandomGridShuffle(grid=(4, 4), p=0.75),
            ...
            A.GaussNoise(p=0.9, var_limit=0.15),
            A.GaussianBlur(blur_limit=(3, 7), p=0.85),
            A.OneOf([
                A.CoarseDropout(max_holes=35, max_height=20, max_width=20, p=1.0),
                A.Downscale(scale_min=0.75, scale_max=0.90, p=1.0),
                A.RandomBrightnessContrast(brightness_limit=0.25, contrast_limit=0.25, p=1.0),
                A.GaussNoise(p=1.0, var_limit=0.15),
            ], p=1.0),
            A.OneOf([
                A.CoarseDropout(max_holes=35, max_height=20, max_width=20, p=1.0),
                A.Downscale(scale_min=0.75, scale_max=0.90, p=1.0),
